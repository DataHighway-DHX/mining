...
contract Lockdrop {
  ...
  // ETH locking events
  event Locked(address indexed owner, uint256 eth, Lock lockAddr, bytes dataHighwayAddr, uint time);
  ...
  /**
    ...
    * @param      dataHighwayAddr   The bytes representation of the target cosmos key
    */
  function lock(bytes calldata dataHighwayAddr)
    external
    payable
    didStart
    didNotEnd
  {
    ...
    emit Locked(msg.sender, msg.value, lockAddr, dataHighwayAddr, now);
  }
