...
const signIn = (req, res) => {
  console.log('signIn with req.body', req.body);
  ...
  if (recovered === req.body.account) {
    console.log('signIn SigUtil successfully verified signer account as: ' + req.body.account);
    
    const token = jwt.sign({ loggedInAs: req.body.account }, secret);
    
    console.log('signIn JWT token: ' + token);
    res.cookie('token', token, { domain: 'localhost', httpOnly: true });
    res.end();
  } else {
    console.log('signIn SigUtil unable to recover the message signer');
  }  
};
